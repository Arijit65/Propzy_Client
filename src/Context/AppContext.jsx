  ...
  const propertyApi = {
    ...
    getAllLocationsProperties: async (params = {}) => {
      try {
        ...
        const url = `http://localhost:5000/api/properties/all-locations${queryParams ? '?' + queryParams : ''}`;
    ...
    getPropertiesByLocation: async (location, params = {}) => {
      try {
        const queryParams = new URLSearchParams(params).toString();
        const url = `http://localhost:5000/api/properties/properties/${location}${queryParams ? '?' + queryParams : ''}`;
        console.log('📡 Calling:', url);
        const res = await axios.get(url);
        console.log('📡 Response:', res.data);
        return res.data;
    ...
    getPropertyById: async (propertyId) => {
      try {
        const url = `http://localhost:5000/api/properties/property/${propertyId}`;
